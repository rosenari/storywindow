...
import {
  CollectPopupSetData,
  ImagePopupSetData,
} from "@/store/action/reducerAction";
  ...
  return (
    <div>
      {Array.from({ length: 14 }).map((_, index) => (
        <div
          key={`firework-${index}`}
          className={styles.firework + " " + `firework-${index + 1}`}
        ></div>
      ))}
    </div>
  );
...
const Popup: React.FC<PopupProps> = ({
  ...
  return (
    <div
      className={[type, styles.main].join(" ")}
      onClick={() => setVisible(false)}
    >
      <div
        className={[type, styles.popup].join(" ")}
        onClick={(e) => e.stopPropagation()}
        style={{ width: size.width, height: "0px" }}
      >
        {children}
      </div>
      {firework && <Fireworks />}
...
export const CollectPopup = () => {
  ...
  const visible =
    useSelector((state: any) => state?.reducer?.collect_popup?.visible) ||
    false;
  const is_mobile = useIsMobile();
  ...
  return (
    ...
    >
      ...
      <div>
        <CollectBody type={type} visible={visible} setVisible={setVisible} />
      </div>
      <div>
...
export const ImagePopup = () => {
  const dispatch = useDispatch();
  const visible =
    useSelector((state: any) => state?.reducer?.image_popup?.visible) || false;
  const imageUrl =
    useSelector((state: any) => state?.reducer?.image_popup?.img_url) || "";
  const setVisible = useCallback((v: boolean) => {
